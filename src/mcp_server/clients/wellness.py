...
from mcp_server.clients.cache import cache_key, cache_ttl
from mcp_server.clients.parser import extract_items, parse_response
from mcp_server.clients.shared_state import MemorySharedState, SharedState
from mcp_server.clients.validation import (
    VALID_IMAGE_YN,
...
from mcp_server.config.settings import BASE_URL
...
class WellnessClient:
    ...
    def __init__(
        ...
        self.shared_state = shared_state or MemorySharedState()
    ...
    async def _get(self, endpoint: str, params: dict) -> dict:
        ...
        cached = await self.shared_state.cache_get(key)
        ...
        async with self.shared_state.fetch_lock(key) as cache_lease:
            cached = await cache_lease.cache_get(key)
            if cached is not None:
                return cached
            ...
            await cache_lease.cache_set(key, result, cache_ttl(key))
            return result
